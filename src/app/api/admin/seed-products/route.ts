...
const MOCK_PRODUCTS = [
  // 1. Phones & Consumer Tech / Electronics / Computing
  {
    ...
    price: 1850,
    compareAtPrice: 2100,
    description: "Brand new sealed Samsung Galaxy A15 with 4GB RAM, 128GB ROM, 50MP triple camera, 5000mAh battery. 1-year official warranty in Tamale.",
    images: [
...
export async function GET() {
  try {
    ...
    const storeCoordinates = (store.address as any)?.location?.coordinates || [-0.852, 9.39];
    ...
    return NextResponse.json({
      success: true,
      message: `Seeded ${upsertedCount} products across all departments! Total published: ${total}`,
      count: total,
    });
  } catch (error: any) {
